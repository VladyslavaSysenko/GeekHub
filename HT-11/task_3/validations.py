...
class Validation:
    ...
    has_needed_banknotes(needed_money: int)
    ...
    def has_needed_banknotes(self, needed_money: int) -> int | Literal[True]:
        """Check if atm has banknotes for needed amount. If not, returns closest smaller available amount"""

        banknotes = Helper(self.db).get_needed_banknotes(needed_money=needed_money)
