...
#     - збережіть отриманий PDF файл у форматі <номер чеку>_robot в директорію output. Окремо зображення
#       робота зберігати не потрібно. Тобто замість зображень у вас будуть pdf файли які містять зображення з чеком.
...
class OrderRobots:
    ...
    def order_robots(self) -> None:
        ...
        for order_info in orders_info:
            ...
            self._order_robot()
            receipt_html = self._get_receipt_html()
            receipt = self._get_receipt()
            self._save_robot_pdf(
                robot_image=robot_image, receipt_html=receipt_html, receipt=receipt
            )
    ...
    def _order_robot(self) -> None:
        ...
        def _try_order() -> bool:
            ...
            # Check if ordered
            result = self._driver_wait().until(
                EC.any_of(
            ...
            return True if result.get_attribute("class") == "alert alert-success" else False
    ...
    def _get_receipt_html(self) -> str:
        """Get HTML of receipt for ordered robot"""

        receipt = self._driver_wait().until(EC.visibility_of_element_located((By.ID, "receipt")))
        return receipt.text
    ...
    def _save_robot_pdf(self, robot_image: Image, receipt_html: str, receipt: str) -> None:
        """Save robot image and receipt HTML as PDF to folder"""
        ...
        # Adjust image size for pdf
        width = robot_image.width / 4
        height = robot_image.height / 4

        # Add image to PDF after receipt text
        pdf.image(robot_image, x=(pdf.w - width) / 2, y=pdf.get_y() + 10, w=width, h=height)

        # Save PDF to a folder
        pdf.output(os.path.join(self.output_folder, f"{receipt}_robot.pdf"))
